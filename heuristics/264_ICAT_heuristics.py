...
def infotodict(seqinfo):
    ...
    for idx, s in enumerate(seqinfo):
        ...
        elif (s.dim4 == 612) and (('REST' in s.protocol_name)):
            info[rest].append({'item': s.series_id})
        elif (s.dim4 == 1) and (('REST' in s.protocol_name)):
            if s_next.dim4 == 612:
                info[sbref_rest].append({'item': s.series_id})
        elif (s.dim4 == 376):
            if 'BED_TASK1_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'BED1'})
            elif 'BED_TASK2_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'BED2'})
            elif 'BED_TASK3_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'BED3'})
            elif 'BED_TASK4_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'BED4'})
            elif 'BED_TASK5_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'BED5'})
            elif 'BED_TASK6_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'BED6'})
        elif (s.dim4 == 1):
            if ('BED_TASK1_PA' in s.protocol_name):
                if s_next.dim4 == 376:
                ...
                if s_next.dim4 == 376:
                    info[sbref_task].append({'item': s.series_id, 'acq': 'BED5'})
            elif ('BED_TASK6_PA' in s.protocol_name):
                if s_next.dim4 == 376:
                    info[sbref_task].append({'item': s.series_id, 'acq': 'BED6'})
